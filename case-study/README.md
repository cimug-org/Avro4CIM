...
- [8. Example SAR Profile JSON Payloads](#8-example-sar-profile-json-payloads)
  ...
  - [8.2. SecurityAnalysisResult_EXTENDED JSON](#82-securityanalysisresult_extended-json)
...
Extract domain:    ap-voc.cim4.eu
                   ↓
Reverse DNS:       eu.cim4.ap_voc (replace dashes and other invalid namespace characters with underscores)
                   ↓
...
### 7.10. Code Generation and CIM Naming Conventions Preservation

**Decision: Preserve CIM Names in Avro Schema, Transform at Code Generation**

For this case study, CIM naming conventions are preserved in the Avro schema to maintain semantic fidelity with the canonical model. Language-specific name transformations are the responsibility of code generation tooling, not the schema itself.

**Key Advantages:**
- **Standards integrity** - Schema reflects IEC CIM standard as defined. CIM is an IEC standard. The Avro schema should reflect the standard as defined, not a language-specific adaptation. This is analogous to:  
  - XML Schemas use the standard's namespace/element names
  - JSON Schemas use the standard's property names
  - Protocol Buffer definitions use the standard's field names
- **Language neutrality** - Schema doesn't favor any particular programming language
  - Different organizations will use different languages:
    - Utility A: Java-heavy
    - Utility B: Python-heavy  
    - Vendor C: C#-heavy
    - TSO: Mixed environment
   - By keeping CIM names in the schema, each organization can apply their own transformations without forcing a lowest-common-denominator compromise within the generated Avro schema.
- **Organizational flexibility** - Each implementer chooses transformation approach
- **Code generation optional** - Supports both generated and generic/dynamic Avro APIs
- **Future-proof** - New languages or convention changes don't require schema updates
- **Full traceability** - Field names match canonical model exactly

**Naming Convention Preservation:**

CIM naming is preserved exactly as defined in the canonical model:
- **Classes**: PascalCase (e.g., `SecurityAnalysisResult`, `Header`)
- **Associations**: PascalCase (e.g., `PowerFlowResult`, `RemedialAction`, `ACDCTerminal`)
- **Attributes**: camelCase (e.g., `atTime`, `isViolation`, `valueA`)
...
```json
{
  ...
  "fields": [
    ...
    {
      "name": "PowerFlowResult",  // ← CIM association name preserved
      "type": ["BaseCasePowerFlowResult", "ContingencyPowerFlowResult"],
      "modelReference": "https://cim4.eu/ns/nc#RemedialActionApplied.PowerFlowResult"
    },
    {
      "name": "RemedialAction",  // ← CIM association name preserved
      "type": "string",
      "modelReference": "https://cim4.eu/ns/nc#RemedialActionApplied.RemedialAction"
    }
  ]
...
| Language | Class Convention | Field Convention | 
|----------|------------------|------------------|
| Java | PascalCase | camelCase |
| Python | PascalCase | snake_case |
| C# | PascalCase | PascalCase |
| JavaScript | PascalCase | camelCase |
| Go | PascalCase | camelCase (exported) |

**Code Generation Approaches:**

Organizations have multiple options for handling name transformations:

**Option 1: Use Standard Tools** (CIM names in generated code)
```bash
# Standard avro-tools
...
Generates:
```java
public class RemedialActionApplied {
    private Object PowerFlowResult;  // ← CIM name preserved
    public Object getPowerFlowResult() { ... }
}
```

**Pros**: Simple, no custom tooling  
**Cons**: Non-conventional field names in Java (but technically will work without modification)

**Option 2: Custom Code Generator Wrappers** (Language-conventional names)

Organizations can create language-specific wrappers that transform names:

```bash
# Custom wrapper for Java
./avro-to-java-pep8.sh SecurityAnalysisResult.avsc ./output/
```

Generates:
```java
public class RemedialActionApplied {
    private Object powerFlowResult;  // ← Transformed to camelCase
    public Object getPowerFlowResult() { ... }
}
```

**Pros**: Conventional code in target language  
**Cons**: Requires custom tooling

**Option 3: Generic/Dynamic APIs** (No code generation)
```java
// Java generic API
GenericRecord record = new GenericData.Record(schema);
record.put("PowerFlowResult", result);  // Use CIM names directly
```

```python
# Python dynamic API
record = {
    "PowerFlowResult": result  # Use CIM names directly
}
```

**Pros**: No code generation needed, works identically across languages  
**Cons**: No type safety or IDE support

**Industry Precedent:**

This approach follows common industry standards for multi-language schema formats:

- **Protocol Buffers**: Schema uses source naming; code generators transform per language
- **Apache Thrift**: Schema uses IDL naming; compilers generate language-conventional code  
- **gRPC**: Preserves source naming in schema 
- **OpenAPI/Swagger**: Schema uses API naming; code generators adapt to language conventions
- **SOAP/WSDL**: Schema uses XML naming; code generators transform to language conventions

**Alternative Approach:**

Schemas could transform names to a "lowest common denominator" convention (typically camelCase) to optimize for immediate usability with standard code generation tools. However, this:

- Obscures the relationship to the canonical CIM model
- Favors certain languages (Java, JavaScript) over others (Python, C#)
- Prevents organizations from choosing their own transformation strategy
- Requires the schema to "pick a side" in multi-language environments

The preservation approach maintains CIM as the authoritative source while enabling flexible implementation strategies across diverse technology ecosystems.
...
```json
{
  "header": {
    "profProfile": "https://ap-voc.cim4.eu/SecurityAnalysisResult/2.4",
    "identifier": "sar-minimal-example-001",
    "isVersionOf": null,
    "version": null,
